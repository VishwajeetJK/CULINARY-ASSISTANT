def my_food_recommendation(food_name):
    ...
    df1=pd.read_csv('../input/fooddata/FOOD RECOMMENDATION SYSTEM - Sheet1.csv')
    df1.columns = ['food_id','title','canteen_id','price', 'num_orders', 'category', 'avg_rating', 'num_rating', 'tags', 'imgsrc']
    ...
    df1['referal name'] = df1.apply(create_referal, axis=1)
    #df1.head(15)
    ...
    ans=df1.loc[get_recommendations(title=food_name)]
    a=ans.iloc[:,1].values
    b=ans.iloc[:,9].values
